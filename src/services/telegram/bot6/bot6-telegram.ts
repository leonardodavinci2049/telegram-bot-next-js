...
const BOT6_CONFIG_ID = 10;

let bot: Bot | null = null;

async function ensureBot(): Promise<Bot> {
  if (!bot) {
    const botConfig = await getTelegramBotDbConfig(BOT6_CONFIG_ID);
...
export async function registerWebhook(): Promise<void> {
  ...
  const botConfig = await getTelegramBotDbConfig(BOT6_CONFIG_ID);
  const webhookUrl = `${botConfig.WEBHOOK_URL}/api/bot6-telegram/webhook`;

  try {
    await b.api.setWebhook(webhookUrl);
  } catch (error) {
    console.error("[telegram:bot6] Failed to register webhook:", error);
    throw error;
  }
}
